...
- **`POST /chat/texto` (novo)**: pergunta digitada (campo `texto`) → mesmo fluxo
  RAG/multiagente com **`com_audio=False`** → **JSON** `{transcricao, resposta,
  abstencao, referencias, latencia_s}` — sem TTS (resposta em texto puro).
- **Endpoints JSON removidos na v0.4**: `/rag/perguntar` e `/rag/imagem/analisar`
  **não eram usados pelo `index.html`** e foram removidos; depois, `/chat/imagem`
  e `/chat/texto` foram **unificados em `POST /chat`** (áudio/imagem/texto) e
  `/voz/saude`/`/rag/saude` foram removidos — saúde fica **somente em
  `GET /saude`** (payload documentado no Swagger).
- Rate limit: apenas o `/chat` (único endpoint generativo) fica no grupo `rag` (4/min).
